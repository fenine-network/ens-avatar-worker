...
describe("Header Routes", () => {
  ...
  describe("GET /:name/h", () => {
    ...
    test("updates an existing image with a new one", async () => {
      ...
      await env.HEADER_BUCKET.put(media.MEDIA_BUCKET_KEY.registered("sepolia", MOCK_NAME), initialImage, {
        httpMetadata: { contentType: "image/jpeg" },
      });
      ...
      await env.HEADER_BUCKET.put(media.MEDIA_BUCKET_KEY.registered("sepolia", MOCK_NAME), updatedImage, {
        httpMetadata: { contentType: "image/jpeg" },
      });
    ...
    test("works across different networks", async () => {
      ...
      await env.HEADER_BUCKET.put(media.MEDIA_BUCKET_KEY.registered("sepolia", MOCK_NAME), sepoliaImage, {
        httpMetadata: { contentType: "image/jpeg" },
      });
